            ...
            if (in_array($relationship, $this->_with)) {
                ...
                if (is_object($row)) {
                    if (isset($options['foreign_key'])) {
                        $row->{$relationship} = $this->{$relationship . '_model'}->get_by($options['foreign_key'], $row->{$options['primary_key']});
                    } else {
            ...
            if (in_array($relationship, $this->_with)) {
                ...
                if (is_object($row)) {
                    if (!isset($options['order_by'])) {
                        $options['order_by'] = $options['primary_key'];
                    }
                    $row->{$relationship} = $this->{$relationship . '_model'}->order_by($options['order_by'])->get_many_by($options['primary_key'], $row->{$this->primary_key});
                } else {
                    if (!isset($options['order_by'])) {
                        $options['order_by'] = $options['primary_key'];
                    }
