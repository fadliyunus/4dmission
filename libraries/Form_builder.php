...
class Form_builder
{
    ...
    private $config = array(/* Config array - can be overrided by passing in array in ini() */
        ...
        'default_control_label_class' => 'col-sm-3 col-form-label',
        'default_no_label_class' => 'col-sm-offset-3',
        'default_form_control_class' => 'col-sm-6',
                ...
                switch ($this->func) {
                    ...
                    case 'form_radio':
                        ...
                        foreach ($all_elm_options['options'] as $elm_suboptions) {
                            ...
                            $this->elm_options['id'] = $elm_suboptions['id']; //$all_elm_options['id'];
                            $this->elm_options['class'] = 'form-check-input';
                            ...
                            $label_class = substr($this->func, 5);
                            array_key_exists('disabled', $this->elm_options) && $label_class .= ' disabled';

                            $this->print_string .= '<div class="form-check">';
                            $this->print_string .= $this->_build_input(FALSE);
                            $this->print_string .= '<label class="form-check-label" for="'.$this->elm_options['id'].'">';
    ...
    function build_display($options, $data_source = array())
    {
        ...
        foreach ($options as $elm_options) {
            if (!isset($elm_options['type'])) {
                $elm_options['type'] = 'input';
            }
            $this->elm_options = $elm_options;

            if (is_array($this->elm_options) && $this->elm_options['type'] != 'hidden') {
                $this->_prep_options();
                if ($this->func != 'form_json') {
                    //                    $this->func = 'form_label'; /* The only difference */
                    $this->func = 'form_control_static'; /* The only difference */
                }
                $this->print_string .= $this->_pre_elm();
                $this->print_string .= $this->_label();
                $this->print_string .= $this->_build_input(true, true);
                $this->print_string .= $this->_post_elm();
            }
        }
    ...
    private function _build_input($include_pre_post = true, $display = false)
    {
        $input_html_string = '';
        ...
        if ($this->func == 'form_combine') {
            ...
            switch ($this->func) {
                ...
                case 'form_button':
                    ...
                    $this->elm_options['content'] = $this->elm_options['label'];
                    unset($this->elm_options['label']);

                    $input_html_string = form_button($this->elm_options);
                    break;
                case 'form_anchor':
                case 'form_a':
                    $class = str_replace($this->config['default_button_classes'], '', $this->elm_options['class']);
                    ...
                    break;
                case 'form_control_static':
                    $input_html_string = '<p class="form-control-static" data-display="' . $this->elm_options['id'] . '">' . $this->elm_options['value'] . '</p>';
                    break;
                case 'form_label':
                ...
                case 'form_checkbox':
                    ...
                    break;
                case 'form_radio':
                    $this->elm_options['data-title'] = $this->elm_options['label'];
        ...
        $ret_string .= $this->_build_input_addons_pre($display);
    ...
    private function _build_input_addons_pre($display = false) {
        $ret_string = '';
        if ($this->input_addons['exists']) {
            if (!empty($this->input_addons['pre_html'])) {
                ...
                foreach ($this->input_addons['pre'] as $pre_addon) {
                    $ret_string .= '<div class="input-group-prepend"><span class="input-group-text'.(isset($this->elm_options['input_group_addon_class']) ? ' '.$this->elm_options['input_group_addon_class'] : '').'">' . $pre_addon . '</span></div>';
                }
            }
        }
        return $ret_string;
    ...
    {
        return '<div class="' . $this->config['default_input_container_class'] . (isset($this->elm_options['input_container_class']) ? ' ' . $this->elm_options['input_container_class'] : '') . '">';
    }
    ...
    private function _pre_input()
    {
        ...
        return '<div class="' . (isset($this->elm_options['form_control_class']) ? $this->elm_options['form_control_class'] : $this->config['default_form_control_class']) . '">';
    }
    ...
    private function _label($link_to_input_id = TRUE)
    {
        ...
        return form_label($label, $link_to_input_id ? $this->elm_options['name'] : '', array(
            'class' => isset($this->elm_options['control_label_class']) ? $this->elm_options['control_label_class'] : $this->config['default_control_label_class']
        ));
    }
